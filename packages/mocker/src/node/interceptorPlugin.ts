      ...
      server.ws.on('vitest:interceptor:register', (event: MockedModuleSerialized) => {
        if (event.type === 'manual') {
          ...
          if (event.type === 'redirect') {
            ...
            // `normalizePath` collapses the separators `node:path/posix` leaves
            // behind on Windows, so `..\` cannot survive the check below
            const redirect = normalizePath(join(server.config.root, redirectUrl.pathname))
